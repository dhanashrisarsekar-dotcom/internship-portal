            ...
            <div className="space-y-4">
              {filtered.map((job, idx) => (
                <React.Fragment key={job.id}>
                  <JobCard job={job} />
                  {/* Register CTA after 5th card */}
                  {idx === 4 && (
                    <div className="bg-white border border-gray-200 rounded-xl p-6">
                      <h3 className="font-bold text-gray-800 text-[16px] mb-3">Register now to</h3>
                      <ul className="space-y-2 mb-5">
                        {["Find all work from home jobs", "Get contacted by top recruiters", "Access free AI-powered tools"].map((item, i) => (
                          <li key={i} className="flex items-center gap-2 text-[13px] text-gray-700">
                            <span className="text-[#008bdc] font-bold text-base leading-none">✔</span> {item}
                          </li>
                        ))}
                      </ul>
                      <div className="flex flex-col sm:flex-row gap-3">
                        <button className="flex-1 bg-[#008bdc] text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:bg-[#0079c0] transition text-[14px]">
                        ...
                        <button className="flex-1 border-2 border-gray-300 text-gray-700 py-3 rounded-lg font-bold flex items-center justify-center gap-2 hover:border-[#008bdc] hover:text-[#008bdc] transition text-[14px]">
                          ✉ Continue with Email
                        </button>
                      </div>
                      <p className="text-[11px] text-gray-400 mt-3 text-center">
                        By continuing, you agree to our <span className="underline cursor-pointer">T&C.</span>
                      </p>
                    </div>
                  )}
                </React.Fragment>
